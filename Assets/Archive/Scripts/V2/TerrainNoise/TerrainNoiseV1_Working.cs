...
public class TerrainNoiseV1_Working : MonoBehaviour {
	...
	void RegenerateMesh(Mesh mesh) {
		if (mesh == null || prevTilesX != tilesX || prevTilesZ != tilesZ) {
			prevTilesX = tilesX;
			prevTilesZ = tilesZ;

			GetComponent<FlatMeshGeneratorV4_Working> ().GenerateMesh (mesh);
		}
